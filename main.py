...
def lade_daten(args: argparse.Namespace, etappe: Dict[str, Any], config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        if args.input:
            ...
            daten = hole_wetterdaten(etappe["punkte"], modus=args.modus)
        ...
        if not isinstance(wetter, dict):
            raise DataError("Ungültiges Datenformat: 'wetter' ist kein Dictionary")
        return daten
    except json.JSONDecodeError as e:
        raise DataError(f"Ungültiges JSON-Format: {str(e)}")
    except OSError as e:
        raise DataError(f"Fehler beim Lesen der Datei: {str(e)}")
    except Exception as e:
        raise DataError(f"Unerwarteter Fehler beim Laden der Daten: {str(e)}")

def wetter_dict_fuer_inreach(wetter: dict) -> dict:
...
def generiere_bericht(
    ...
    try:
        if args.inreach:
            return generiere_kurznachricht(wetter_dict_fuer_inreach(wetter)), True
        else:
